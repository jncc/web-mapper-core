...
echo "Running Deploy to Live Task on EMODnet server...."
...
echo "Installing Correct Configuration on EMODnet Live Server and Restarting Process..."
rc="ssh $user@$livehost \"cd $remotepath/$project && cp -f appsettings.emod.json appsettings.json && cp -f appsettings.emod.json appsettings.Development.json && rm -f appsettings.mpa.json && kill \\\$(pgrep -f -o '$project')\""
echo $rc
ssh $user@$host "$rc"
